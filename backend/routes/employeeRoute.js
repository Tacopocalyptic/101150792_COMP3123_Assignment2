...
router
  ...
  .post(
    [
      ...
      body("salary", "Salary must be integer value, minimum 0")
        .optional()
        .isInt({ min: 0 }),
      body("date_of_joining", "Must be date format")
        .optional()
        .isISO8601()
        .toDate(),
  ...
  .delete([query("eid").notEmpty()], async (req, res) => {
    ...
    if (!eid) {
      return res
        .status(400)
        .json({ status: false, message: "No employee id sent" });
    }
    ...
    try {
      ...
      if (!emp) {
        return res
          .status(400)
          .json({ status: false, message: `No employee with ID ${eid} found` });
      }
...
router
  ...
  .get(param("eid").isString(), async (req, res) => {
    ...
    if (!eid) {
      return res.status(400).json({ status: false, message: "No employee id" });
    }
  ...
  .put(
    ...
    async (req, res) => {
      ...
      if (!eid || !data) {
        return res
          .status(400)
          .json({ status: false, message: "Employee parameters missing" });
      }
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
